...
class dataAkun_p extends StatefulWidget {
  const dataAkun_p({ Key? key }) : super(key: key);

  @override
  State<dataAkun_p> createState() => _dataAkun_pState();
}

class _dataAkun_pState extends State<dataAkun_p> {
...
Stream<List<Data>> readData() => FirebaseFirestore.instance.collection("Akun_Pemilik").where("Email", isEqualTo:
...
Future updateData (Data data) async {
  final docData = FirebaseFirestore.instance.collection("Akun_Pemilik").doc(data.ID);
  data.ID = docData.id;
